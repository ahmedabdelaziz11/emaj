...
<div class="breadcrumb-header justify-content-between">
	<div class="my-auto">
		<div class="d-flex">
			<h4 class="content-title mb-0 my-auto"><a href="{{ url('/' . $page='returned-receipts') }}">فواتير مرتجع المشتريات</a></h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/
			اضافة فاتورة</span>
		</div>
	</div>
</div>
...
<div class="row row-sm">
	<div class="col-xl-12">
		...
		<div class="card mg-b-20" id="tabs-style2">
			<div class="card-body">
				<form action="{{ route('returned-receipts.store') }}" method="post" autocomplete="off">
					@csrf
					<div class="d-flex justify-content-center">
						<h1>فاتورة مرتجع مشتريات</h1>
					</div>
					<br>
					<div class="row">
						...
						<div class="form-group col-8">
							...
							<select class="form-control select2 " id="invoice_id" name="receipt_id" required>
...
<script>
	$(document).ready(function() {
		$('select[name="receipt_id"]').on('change', function() {
			...
			if (invoice_id) {
				$.ajax({
					url: "{{ URL::to('receipts') }}/" + invoice_id,
					type: "GET",
					dataType: "json",
					success: function(data) {
						...
						data.forEach( (product) => {
							$("#items_container").append(`
								...
								<td style="padding-top:20px;font-size:17px;font-weight:bold"><input type="hidden" name="id[]" value="` + product.id + `" min="1"><input type="number" style="font-size:17px;font-weight:bold" name="quantity[]" onchange='updateTotal2();' value="` + product.pivot.product_quantity + `" class=" form-control text-center " max="` + product.pivot.product_quantity + `" min="0"></td>
								</tr>
							`);
						});
					},
