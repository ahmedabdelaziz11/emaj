    ...
    <div class="col-xl-12">
        <div class="card mg-b-20">
            <div class="card-header pb-0" id="form">
                ...
                <form action="{{route('project-statement.store')}}" method="POST" role="search" autocomplete="off">
                    {{ csrf_field() }}
                    <div class="row">
                        <div class="col">
                            <label for="exampleFormControlSelect1">المركز</label>
                            <select class="form-control select2 " name="cost_id" required>
                                <option value="">اختر المركز</option>
                                @foreach ($costs as $cost)
                                    <option @if(isset($c_cost) && $c_cost->id == $cost->id ) selected @endif value="{{ $cost->id }}">{{ $cost->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <!-- <div class="col">
                        ...
                        <div class="col">
                            <label for="exampleFormControlSelect1">نوع التقرير</label>
                            <select class="form-control " name="level" required>
                                <option value="">اختر النوع </option>
                                <option @if(isset($data) != null  ) selected @endif value="1">عام</option>
                                <option @if(isset($days) != null  ) selected @endif value="2">تفصيلي</option>
                            </select>
                        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mg-t-20">
                            <div class="col-md" style="margin-left: 50px;">
                                <p class="invoice-info-row"style="font-size: 17px;font-weight: bold"><span>اسم المركز</span> <span></span>{{$c_cost->name}}</p>
                            </div>
                            <div class="col-md">
                        ...
                        <div class="table-responsive mg-t-40">
                            <table class="table table-invoice border text-md-nowrap mb-0">
                                    ...
                                    <tbody>
                                        @php
                                            $start_account = $total_d = $total_c = 0;
                                        @endphp
                                        @foreach($days as $x)
                                            @php
                                        ...
                                        <tr class="table-dark" style="color: black;">
                                            ...
                                            <td style="text-align: center;font-size: 17px;font-weight: bold;border: 1px solid rgb(177, 170, 170);">{{number_format($total_d,2)}}</td>
                                            <td style="text-align: center;font-size: 17px;font-weight: bold;border: 1px solid rgb(177, 170, 170);">{{number_format($total_c,2)}}</td>
                                            <td style="text-align: center;font-size: 17px;font-weight: bold;border: 1px solid rgb(177, 170, 170);">{{number_format($start_account,2)}}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            ...
                            <table class="table table-invoice border text-md-nowrap mb-0">
                                    ...
                                    @endphp
                                            ...
                                            <form id="{{$x['id']}}" action="{{route('account-statement.store')}}" method="POST" role="search" autocomplete="off">
                                                ...
                                                <td style="text-align: center;font-size: 17px;font-weight: bold;border: 1px solid rgb(177, 170, 170);">{{number_format($x['debit'],2)}}</td>
                                                <td style="text-align: center;font-size: 17px;font-weight: bold;border: 1px solid rgb(177, 170, 170);">{{number_format($x['credit'],2)}}</td>
                                                <td style="text-align: center;font-size: 17px;font-weight: bold;border: 1px solid rgb(177, 170, 170);">{{number_format($x['debit'] - $x['credit'],2)}}</td>
                                ...
                                <div class="table-responsive mg-t-40">
                                    ...
                                    <tbody class="border border-light tx-right">
                                            <tr class="border border-light tx-right">
                                                <th style=" font-size: 23px;font-weight: bold" class="tx-center"colspan="3">الرصيد</th>
                                                <th style="font-size: 25px" class="tx-right"colspan="1" >{{ number_format($total_acc,2) }}</th>
                                            </tr>
                                            <tr >
                                                <th style="border: none; font-size: 24px; padding-right: 150px;padding-bottom: 40px">توقيع المحاسب</th>
                                                <th style="border: none; font-size: 25px" class="tx-right" colspan="2">توقيع المدير المالى</th>
