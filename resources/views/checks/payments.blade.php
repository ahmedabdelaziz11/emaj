...
<script>
    window.onload = function() {
        notif({
            msg: "فشلت العميلة برجاء اعادة المحولة مع التاكد من تدخيل جميع البايانات",
            type: "error"
        })
    }
</script>
        ...
        <div class="card mg-b-20">
            <div class="card-body">
                <a class="modal-effect btn btn-outline-primary btn-block" data-effect="effect-scale"
                        data-toggle="modal" href="#modaldemo8">اضافة سند صرف</a>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="col-xl-12">
        <div class="card mg-b-20">
            <div class="card-header pb-0">
                <form action="{{route('payment-details')}}" method="GET" role="search" autocomplete="off">
                    ...
                    <table id="example1" class="table key-buttons text-md-nowrap" data-page-length='50'
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="border-bottom-0">رصيد الصرف</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($payments as $x)
                                ...
                                <tr>
                                    ...
                                    <td class="text-primary">المالك</td>
                                    <td>م / محمد</td>
                                    @else
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">اضافة سند صرف</h6><button aria-label="Close" class="close" data-dismiss="modal"
                    ...
                    <form action="{{ route('payment-vouchers.store') }}" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <option value="" disabled selected>اختر النوع </option>
                                ...
                                <option value="3"> نفقة </option>
                                <option value="5"> مرتب </option>
                                <option value="7"> دفعة قرض </option>
                                <option value="6"> مالك (سحب نقدية) </option>
                            </select>
                        </div> 
                        ...
                        <div class="form-group" id="salary_div" style="display: none;">
                            <label for="inputName" class="control-label">المرتب</label>
                            <input type="number" class="form-control" id="salary" name="salary"
                            value="0" min="0" step="1"  title="يرجي ادخال القيمة">
                        </div>

                        <div class="form-group" id="penalty_div" style="display: none;">
                            <label for="inputName" class="control-label">خصم الجزاءات</label>
                            <input type="number" class="form-control" id="penalty" name="penalty"
                            value="0" min="0" step="1"  title="يرجي ادخال القيمة">
                        </div>

                        <div class="form-group" id="debt_div"  style="display: none;">
                            <label for="inputName" class="control-label">خصم السلف</label>
                            <input type="number" class="form-control" id="debt" name="debt"
                            value="0" min="0" step="1"  title="يرجي ادخال القيمة ">
                        </div>

                        <div class="row" id="spare" style="display: none;">
                            <div class="col">
                                <label style="margin-left: 20px">النفقة تتبع الضمان ام لا ؟</label>
                                <input name="rdio" id="spare_radio" value="1" type="radio"><span style="margin-left: 20px">نعم </span>
                                <input  name="rdio" value="0" type="radio" checked><span>لا</span>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="exampleModal2" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> تعديل سند صرف </h5>
                ...
                <div class="modal-body">
                    ...
                    <form action="{{route('payment-vouchers.update',1)}}" method="post" autocomplete="off">
                        {{ method_field('patch') }}
                        {{ csrf_field() }}
                        <div class="form-group">
                            <input type="hidden" name="id" id="id" value="">
                            <input type="hidden" name="temp" id="temp" value="">
                            <input type="hidden" name="amount_temp" id="amount_temp" value="">
                            <input type="hidden" name="account_temp" id="account_temp" value="">
                        </div>
    ...
    <div class="modal" id="modaldemo9">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                ...
                <form action="{{route('payment-vouchers.destroy',1)}}" method="post">
                    {{ method_field('delete') }}
                    {{ csrf_field() }}
                    <div class="modal-body">
...
<script>
    $('#exampleModal2').on('show.bs.modal', function(event) {
        ...
        modal.find('.modal-body #amount_temp').val(amount);

        modal.find('.modal-body #account_temp').val(account_id);
    ...
    $(document).ready(function() {
        ...
        $('select[name="type"]').on('change', function() {
            ...
            {  
                ...
                document.getElementById("penalty_div").style.display = "block";
                document.getElementById("debt_div").style.display = "block";
            ...
            if(type != 5 && type != 3){
                ...
                document.getElementById("penalty_div").style.display = "none";
                document.getElementById("debt_div").style.display = "none";
            }
...
<script>
	$(document).ready(function(){
	  $("#account_id").change(function(){
		var amount_temp = parseInt( $('#amount_temp').val());
		var account_temp = parseInt( $('#account_temp').val());
		var account = parseInt($(this).find(":selected").attr('account'));
		var account_id = parseInt($(this).find(":selected").attr('value'));
		if(account_id == account_temp )
		{
			var account2 = account + amount_temp;
			$('input#amount').attr('max',account2);
		}
		else{
			$('input#amount').attr('max',account);
		}
