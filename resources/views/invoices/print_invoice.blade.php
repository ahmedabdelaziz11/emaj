...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto"><a href="{{ url('/' . $page='invoices') }}">فواتير بيع المنتجات </a></h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">طباعه الفاتورة</span>
						</div>
					</div>
								...
								<div class="card-body">
									...
									<div class="row mg-t-20">
										<div class="col-md">
											...
											<p class="invoice-info-row"style="font-size: 17px;font-weight: bold"><span>س-ت</span> <span>{{$offers->client->sgel}}</span><span>ب-ض </span> <span>{{$offers->client->dreba}}</span></p>
										</div>
										<div class="col-md" style="margin-right: 80px">
											...
											<p class="invoice-info-row"style="font-size: 17px;font-weight: bold"><span>تاريخ اصدار الفاتورة</span> <span>{{$offers->invoice_Date}}</span></p>
										</div>
									</div>
										...
										<table class="table table-invoice border text-md-nowrap mb-0">
											...
											<tbody>
												...
												@foreach ($details as $product)
													<?php 
													$i++;
													$totalall = $totalall + $product->product_selling_price ;
													?>
													<tr>
														...
														<td style="text-align: center;font-size: 17px;font-weight: bold;">{{number_format($product->product_quantity * $product->product_selling_price )}}</td>																																						                              											
													</tr>
												@endforeach
											</tbody>
										</table>
											...
											<table  class="table table-invoice border text-md-nowrap mb-0">
												<tbody>
													<tr>
														...
														<th style="font-size: 25px" class="tx-right" colspan="2">{{ number_format($totalall) }}</th>
														<th style="font-size: 32px;" class="tx-right">نسبه الخصم (%)</th>
														<th style="font-size: 23px;font-weight: bold" class="tx-right" colspan="2">{{ number_format($offers->discount) }}</th>
														<th></th><th></th>
														...
														<th style="font-size: 27px;font-weight: bold" class="tx-right" colspan="2">
															<h4 class="tx-primary tx-bold">{{ number_format($offers->total) }}</h4>
														</th>
													</tr>
												</tbody>
											</table>
